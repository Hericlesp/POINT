...
import PABLO_FRONT.Pablo.Projeto_Point as centro
import PABLO_FRONT.Deyu.chamada_adm.
...
class POINT:
  ...
  def fazer_login(self):
    ...
    cursor.execute(
    "SELECT NOME,PERMISSAO FROM login WHERE (CPF = ? OR RA = ?) AND SENHA = ? ",(usuario_int, senha)
    )
    ...
    if resultado:
      ...
      user_permissao = resultado[1]
      
      messagebox.showinfo("Login",f"Bem-Vindo, {user_name.upper()}!")
      
      self.janela.destroy()
      
      if user_permissao == ["ROOT" or "PROFESSOR"]:
        nem_janela = tk.Tk()
        centro.Point(nem_janela).run()
        
      elif user_permissao == "aluno":
        nem_janela = tk.Tk()
